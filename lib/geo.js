...
(function(root, factory) {
    ...
    // var UTILITY_LIBRARY = 'underscore';
    var UTILITY_LIBRARY = 'lodash';
    // var UTILITY_LIBRARY = 'ramda';
    /* istanbul ignore else */
    if (typeof define === 'function' && define.amd) {
        define([UTILITY_LIBRARY], function(_) {
            return (root.geolib = factory(root, _));
        });
    } else if (typeof exports === 'object') {
        var _ = require(UTILITY_LIBRARY);
        module.exports = factory(root, _);
    } else {
        root.geolib = factory(root, _);
    }
    ...
    function emptyValueArray(size) {return _.range(0, size).map(EMPTY_VALUE);}
    ...
    function isObject(value) {return typeof(value) === 'object' && !!value && !Array.isArray(value);}
    ...
    **/
     ...
     * @description Determines if value is valid direction (ex: NSEW)
     * @param {string} value
     * @returns {boolean}
    **/
    var isDirection = geolib.isDirection = _.memoize(function(value) {
        if (!isString(value)) {
            return false;
        }
        var directions = ['north', 'south', 'east', 'west'];
        var dirs = directions.map(function(direction) {return direction.slice(0, 1);});
        return _.union(directions, dirs).indexOf(value.toLowerCase()) > -1;
    });
    /**
     * @function hasDirection
     * @description Returns true if input has direction, false otherwise
     * @param {array.<string, number>} [value]
     * @returns {boolean}
    **/
    var hasDirection = geolib.hasDirection = function(data) {
        data = data ? data : this.get('data');
        ...
        return data.filter(isDirection).length > 0;
    };
    /**
     * @function isValidLatitude
     * @description Returns true if input is valid latitude, false otherwise
     * @param {number} data
     * @returns {boolean}
    **/
    var isValidLatitude = function(data) {
        var latitudeDegrees = 90;
        return _.compose(Math.ceil, Math.abs)(data) in _.range(0, latitudeDegrees + 1);
    };
    /**
     * @function isValidLongitude
     * @description Returns true if input is valid longitude, false otherwise
     * @param {number} data
     * @returns {boolean}
    **/
    var isValidLongitude = function(data) {
        var longitudeDegrees = 180;
        return _.compose(Math.ceil, Math.abs)(data) in _.range(0, longitudeDegrees + 1);
    };
    /**
